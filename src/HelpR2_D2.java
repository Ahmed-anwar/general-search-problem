...
public class HelpR2_D2 extends SearchProblem{
	...
	public void genGrid(){
		...
		for (int i = 0; i < numberOfRocks; i++) {
			...
			while(grid[row][column] != Cell.EMPTY)
			{
				row = (int) (Math.random() * rows);
				column = (int) (Math.random() * columns);
			}

			grid[row][column] = Cell.ROCK;
		}
		...
		for (int i = 0; i < numberOfPads; i++) {
			...
			while(grid[row][column] != Cell.EMPTY && grid[row][column] != Cell.ROCK)
			{
				row = (int) (Math.random() * rows);
				column = (int) (Math.random() * columns);
			}
			...
			{
				grid[row][column] = Cell.UNPRESSED_PAD;
				numUnpressedPads++;
			}

			else if(grid[row][column] == Cell.ROCK)
			{
				grid[row][column] = Cell.PRESSED_PAD;
				numPressedPads++;
			}
		}
		...
		while(grid[portalRow][portalCol] != Cell.EMPTY)
		{
		...
		if(numPressedPads == numberOfPads)
			grid[portalRow][portalCol] = Cell.ACTIVE_PORTAL;
		else
			grid[portalRow][portalCol] = Cell.INACTIVE_PORTAL;
		
		int r2d2Row = (int) (Math.random() * rows);
		int r2d2Col = (int) (Math.random() * columns);

		while(grid[r2d2Row][r2d2Col] != Cell.EMPTY)
		{
